...
export DB_NAME=accelerated

echo "[ACCELERATED/BootstrapPostgres.sh] Installing postgres ..."

apt-get install postgresql postgresql-contrib -y > /dev/null
apt-get install postgresql-contrib libpq-dev -y > /dev/null

echo "[ACCELERATED/BootstrapPostgres.sh] -- Creating database user and granting privileges"
...
echo "[ACCELERATED/BootstrapPostgres.sh] -- Configuring postgres"
...
echo "[ACCELERATED/BootstrapPostgres.sh] -- Restarting postgres service"

su postgres service postgresql restart

echo "[ACCELERATED/BootstrapPostgres.sh] -- Creating & importing database $DB_NAME"

su postgres -c "psql -c \"CREATE EXTENSION pgcrypto;\" -d $DB_NAME"
su postgres -c "createdb $DB_NAME"
su postgres -c "psql -f $DB_NAME.sql -d $DB_NAME"

echo "[ACCELERATED/BootstrapPostgres.sh] -- Installing nodejs postgres drivers"

npm install pg --save --loglevel=error > /dev/null
npm install pg-native --save --loglevel=error > /dev/null

echo "[ACCELERATED/BootstrapPostgres.sh] Finished installing postgres!"
